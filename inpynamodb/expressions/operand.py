...
from inpynamodb.expressions.condition import (
    BeginsWith, Between, Comparison, Contains, Exists, In, IsType, NotExists
)
from inpynamodb.expressions.update import (
    AddAction, DeleteAction, RemoveAction, SetAction
)
from inpynamodb.expressions.util import get_path_segments, get_value_placeholder, substitute_names
...
class _Operand(object):
    ...
    def _to_operand(self, value):
        if isinstance(value, _Operand):
            return value
        from inpynamodb.attributes import Attribute  # prevent circular import -- Attribute imports Path
...
class Value(_NumericOperand, _ListAppendOperand, _ConditionOperand):
    ...
    def __serialize_based_on_type(value):
        from inpynamodb.attributes import _get_class_for_serialize
