...
class TransactGet(Transaction):
    ...
    async def get(self, model_cls, hash_key, range_key=None):
        """
        ...
        :return:
        """
        operation_kwargs = await model_cls.get_operation_kwargs_from_class(hash_key, range_key=range_key)
...
class TransactWrite(Transaction):
    ...
    async def condition_check(self, model_cls, hash_key, range_key=None, condition=None):
        if condition is None:
            raise TypeError('`condition` cannot be None')
        operation_kwargs = await model_cls.get_operation_kwargs_from_class(
            hash_key,
            range_key=range_key,
            condition=condition
        )
    ...
    async def delete(self, model, condition=None):
        operation_kwargs = await model.get_operation_kwargs_from_instance(condition=condition)
    ...
    async def update(self, model, actions, condition=None, return_values=None):
        operation_kwargs = await model.get_operation_kwargs_from_instance(
            actions=actions,
            condition=condition,
            return_values_on_condition_failure=return_values
