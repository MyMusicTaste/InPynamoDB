...
from pynamodb.attributes import UnicodeAttribute, UTCDateTimeAttribute, NumberSetAttribute, UnicodeSetAttribute, \
    BinarySetAttribute, NumberAttribute, BooleanAttribute, TTLAttribute, BinaryAttribute, MapAttribute, ListAttribute, \
    VersionAttribute
from pynamodb.connection.util import pythonic
from pynamodb.constants import ITEM, STRING_SHORT, ATTRIBUTES, EXCLUSIVE_START_KEY, RESPONSES, CAMEL_COUNT, ITEMS, \
    SCANNED_COUNT, LAST_EVALUATED_KEY, REQUEST_ITEMS, UNPROCESSED_KEYS, KEYS, BINARY_SHORT, DEFAULT_ENCODING, \
    UNPROCESSED_ITEMS, ALL, KEYS_ONLY, INCLUDE, MAP_SHORT, LIST_SHORT, NUMBER_SHORT
...
from tests.data import MODEL_TABLE_DATA, DESCRIBE_TABLE_DATA_PAY_PER_REQUEST, SIMPLE_MODEL_TABLE_DATA, \
    CUSTOM_ATTR_NAME_INDEX_TABLE_DATA, GET_MODEL_ITEM_DATA, COMPLEX_TABLE_DATA, COMPLEX_ITEM_DATA, CAR_MODEL_TABLE_DATA, \
    BATCH_GET_ITEMS, CUSTOM_ATTR_NAME_ITEM_DATA, SIMPLE_BATCH_GET_ITEMS, INDEX_TABLE_DATA, LOCAL_INDEX_TABLE_DATA, \
    BINARY_ATTR_DATA, SERIALIZED_TABLE_DATA, COMPLEX_MODEL_SERIALIZED_TABLE_DATA, COMPLEX_MODEL_ITEM_DATA, \
    OFFICE_EMPLOYEE_MODEL_TABLE_DATA, GROCERY_LIST_MODEL_TABLE_DATA, OFFICE_MODEL_TABLE_DATA, FULL_CAR_MODEL_ITEM_DATA, \
    GET_OFFICE_EMPLOYEE_ITEM_DATA, GET_OFFICE_EMPLOYEE_ITEM_DATA_WITH_NULL, GET_GROCERY_LIST_ITEM_DATA, \
    GET_OFFICE_ITEM_DATA, COMPLEX_MODEL_TABLE_DATA, CAR_MODEL_WITH_NULL_ITEM_DATA, \
    INVALID_CAR_MODEL_WITH_NULL_ITEM_DATA, BOOLEAN_MODEL_TABLE_DATA, BOOLEAN_MODEL_FALSE_ITEM_DATA, \
    ...
    DOG_TABLE_DATA, VERSIONED_TABLE_DATA, BOOLEAN_MODEL_TRUE_ITEM_DATA, TREE_MODEL_TABLE_DATA, TREE_MODEL_ITEM_DATA
...
class ModelTestCase(TestCase):
    ...
    async def init_table_meta(model_clz, table_data):
        with patch(PATCH_METHOD) as req:
            ...
            await (await model_clz._get_connection()).describe_table()
    ...
    async def test_create_model(self):
        ...
        with patch(PATCH_METHOD) as req:
            ...
            self.assertEqual((await BillingModeOnDemandModel._connection.get_meta_table()).data
    ...
    async def test_complex_key(self):
        ...
        """
        await self.init_table_meta(ComplexKeyModel, COMPLEX_TABLE_DATA)
    ...
    async def test_query_limit_greater_than_available_items_single_page(self):
        ...
        with patch(PATCH_METHOD) as req:
            ...
            iterator = await UserModel.query('foo', limit=25)
    ...
    async def test_query_limit_identical_to_available_items_single_page(self):
        ...
        with patch(PATCH_METHOD) as req:
            ...
            req.return_value = {'Count': len(items), 'ScannedCount': len(items), 'Items': items}
            iterator = await UserModel.query('foo', limit=5)
            results = [item async for item in iterator]
            self.assertEqual(len(results), 5)
    ...
    async def test_query_limit_less_than_available_items_multiple_page(self):
        ...
        with patch(PATCH_METHOD) as req:
            ...
            self.assertEqual(await results_iter.last_evaluated_key, {'user_id': items[24]['user_id']})
    ...
    async def test_query_limit_greater_than_available_items_multiple_page(self):
        ...
        with patch(PATCH_METHOD) as req:
            ...
            self.assertEqual(await results_iter.last_evaluated_key, None)
            self.assertEqual(results_iter.total_count, 30)
            self.assertEqual(results_iter.page_iter.total_scanned_count, 60)
    ...
    async def test_query_with_exclusive_start_key(self):
        ...
        with patch(PATCH_METHOD) as req:
            ...
            self.assertEqual(await results_iter.last_evaluated_key, {'user_id': items[9]['user_id']})

            results = [item async for item in results_iter]
            self.assertEqual(len(results), 10)
            self.assertEqual(len(req.mock_calls), 1)
            self.assertEqual(req.mock_calls[0][1][1]['Limit'], 10)
            self.assertEqual(await results_iter.last_evaluated_key, {'user_id': items[19]['user_id']})
    ...
    async def test_query(self):
        ...
        with pytest.raises(ValueError):
            _ = [item async for item in await UserModel.query(
        ...
        with patch(PATCH_METHOD) as req:
            ...
            await (await CustomAttrNameModel._get_connection()).describe_table()
        ...
        with patch(PATCH_METHOD) as req:
            ...
            with self.assertRaises(ValueError):
                async with UserModel.batch_write(auto_commit=False) as batch:
                    ...
                    with pytest.raises(ValueError):
                        await batch.save(UserModel('asdf', '1234'))
    ...
    async def test_index_queries(self):
        ...
        with patch(PATCH_METHOD) as req:
            ...
            await (await IndexedModel._get_connection()).describe_table()

        with patch(PATCH_METHOD) as req:
            req.return_value = LOCAL_INDEX_TABLE_DATA
            await (await LocalIndexedModel._get_connection()).describe_table()
    ...
    async def test_complex_model_retrieve_from_db(self):
        ...
        with patch(PATCH_METHOD, new=fake_db) as req:
            ...
            self.assertEqual(item.key,
                             int(COMPLEX_MODEL_ITEM_DATA.get(ITEM).get('key').get(NUMBER_SHORT)))
    ...
    async def test_car_model_retrieve_from_db(self):
        ...
        with patch(PATCH_METHOD, new=fake_db) as req:
            ...
            self.assertEqual(item.car_id,
                             int(FULL_CAR_MODEL_ITEM_DATA.get(ITEM).get('car_id').get(NUMBER_SHORT)))
    ...
    async def test_deserializing_bool_false_works(self):
        fake_db = self.database_mocker(BooleanModel,
                                       ...
                                       BOOLEAN_MODEL_FALSE_ITEM_DATA, 'user_name', 'S', 'alf')
        with patch(PATCH_METHOD, new=fake_db) as req:
    ...
    async def test_deserializing_new_style_bool_true_works(self):
        fake_db = self.database_mocker(BooleanModel,
                                       BOOLEAN_MODEL_TABLE_DATA,
                                       BOOLEAN_MODEL_TRUE_ITEM_DATA, 'user_name', 'S', 'justin')
        with patch(PATCH_METHOD, new=fake_db) as req:
    ...
    async def test_deserializing_map_four_layers_deep_works(self):
        fake_db = self.database_mocker(TreeModel,
                                       ...
                                       TREE_MODEL_ITEM_DATA, 'tree_key', 'S', '123')
        with patch(PATCH_METHOD, new=fake_db) as req:
